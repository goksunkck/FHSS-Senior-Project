...
class RAMDataset(Dataset):
    def __init__(self, h5_filepath, sequence_starts, labels, lookback_window):
        ...
        with h5py.File(h5_filepath, 'r') as f:
            ...
            if 'SNR' in f:
                ...
                print("Warning: SNR data not found in HDF5.")
    ...
    def __getitem__(self, idx):
        ...
        # SNR is constant for the whole signal, check SNR at start
        # The SNR array is aligned with X_data.
        snr_val = self.snr_data[start][0]
        
        return seq, torch.tensor(label, dtype=torch.long), torch.tensor(snr_val, dtype=torch.float)

def main():
    print("--- Testing M-Sequence Transformer on UNSEEN SEEDS ---")
    ...
    plt.plot(sorted_snrs, snr_accs, marker='o', linewidth=2)
    plt.title(f'Prediction Accuracy vs SNR (Overall: {acc:.1f}%)')
    plt.xlabel('SNR (dB)')
    plt.ylabel('Accuracy (%)')
    plt.grid(True)
    plt.ylim(0, 100)
    plt.savefig('results/mseq_accuracy_vs_snr.png')
    print("Saved results/mseq_accuracy_vs_snr.png")
    ...
    plt.ylabel('True')
    plt.savefig('results/test_mseq_confusion.png')
    print("Saved results/test_mseq_confusion.png")
