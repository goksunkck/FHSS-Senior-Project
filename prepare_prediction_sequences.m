...
% Define classes based on the system parameters, not the data content.
% This ensures the list of classes is always complete.
k = 3; % From simParams in the generation script
numChannels = 2^k;
class_values = uint8(0:numChannels-1)';
class_names = string(class_values); % Creates ["0"; "1"; "2"; ...]

rng(42);
cv = cvpartition(total_sequences, 'HoldOut', validation_split_ratio);
...
fprintf('Train sequences: %d | Validation sequences: %d\n', numel(YTrain), numel(YValidation));
