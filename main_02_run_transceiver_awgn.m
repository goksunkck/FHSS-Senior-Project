...
simParams.fs = 60e6; % Sample Rate (Hz)
simParams.snr_dB = -7; % Signal-to-Noise Ratio in dB (Adjusted for new PG)
...
simParams.symbolRate = simParams.fs * 0.1; % 6e6 sym/s (6 Msym/s)

% We'll use non-coherent FSK with h=1 (robust)
% So, freqSeparation = symbolRate
simParams.freqSeparation = simParams.symbolRate; % 6 MHz
...
simParams.pnPoly = [3 2 0];
simParams.pnInitial = [0 0 1];
...
spacing = simParams.freqSeparation; % 6 MHz spacing
% **FIX:** Center the 8 channels (42MHz total band) inside the valid
% [-27, +27]MHz range.
% baseFreq = -21e6; % This centers the hopset   
baseFreq = -((numChannels-1) * spacing) / 2; % -21e6
simParams.hopset = (0:numChannels-1) * spacing + baseFreq;
% hopset = [-21e6, -15e6, -9e6, -3e6, 3e6, 9e6, 15e6, 21e6]
...
[~, dehoppedSignal] = createReceiver(receivedSignal, simParams, hopFrequencies);
ax2 = subplot(2, 1, 2); 
...
title('Receiver''s De-hopped Signal (Noisy)');
xlabel('Time (s)');
ylabel('Amplitude');
grid on;
